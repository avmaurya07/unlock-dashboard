...
export default function JobCategories() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/api/admin/job-categories?${params.toString()}`);
  ...
  const save = async () => {
    ...
    try {
      if (editId) {
        await api.patch(`/api/admin/job-categories/${editId}`, { name, description });
        toast.success("Category updated");
      } else {
        await api.post("/api/admin/job-categories", { name, description });
  ...
  const toggleActive = async (c) => {
    try {
      await api.patch(`/api/admin/job-categories/${c._id}`, { isActive: !c.isActive });
  ...
  const remove = async (id) => {
    ...
    try {
      await api.delete(`/api/admin/job-categories/${id}`);
  ...
  return (
    <div className="container-fluid">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div>
          <h3 className="mb-0">Job Categories</h3>
          <small className="text-muted">Manage categories for job postings</small>
        </div>
