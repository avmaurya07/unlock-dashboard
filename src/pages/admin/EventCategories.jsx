...
export default function EventCategories() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/api/admin/event-categories?${params.toString()}`);
  ...
  const save = async () => {
    ...
    try {
      if (editId) {
        await api.patch(`/api/admin/event-categories/${editId}`, { name, description });
        toast.success("Category updated");
      } else {
        await api.post("/api/admin/event-categories", { name, description });
  ...
  const toggleActive = async (c) => {
    try {
      await api.patch(`/api/admin/event-categories/${c._id}`, { isActive: !c.isActive });
  ...
  const remove = async (id) => {
    ...
    try {
      await api.delete(`/api/admin/event-categories/${id}`);
    ...
    <div className="container-fluid">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div>
          ...
          <small className="text-muted">Manage categories for event listings</small>
        </div>
